...
const FeedService = {
  ...
  getPersonalizedFeed: async (
    ...
    try {
      ...
      if (params.reelsOnly)
        queryString.append("reelsOnly", params.reelsOnly.toString())
      if (params.subCategory)
        queryString.append("subCategory", params.subCategory)
  ...
  unfollowStore: async (storeId: string) => {
    try {
      const response = await api.delete("/feed/follow-store", {
        data: { storeId },
      })
      return response.data
    } catch (error: any) {
